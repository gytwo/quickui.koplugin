...
local function make_path_durable(path)
    ...
    while current and current ~= "" and not exists(current) do
        missing[#missing + 1] = current        local parent = ffiutil.dirname(current)
...
function M.moveEpubBackup(from, to)
    ...
    local source = from .. ".quickui-metadata.bak"
    local source_snapshot = source .. ".json"
    local destination = to .. ".quickui-metadata.bak"
...
function M.load(file)
    ...
    if type(file) ~= "string" or lfs.attributes(file, "mode") ~= "file" then
        logger.warn("[QuickUI metadata] Service.load: invalid_file")
        return nil, "invalid_file"
    end
    if is_current_reader_file(file) then
        logger.warn("[QuickUI metadata] Service.load: open_book")
        return nil, "open_book"
    end

    local original
    if M.isEpub(file) then
        local props, err = require("qui_metadata.qm_epub").read(file)
...
function M.save(file, draft, options)
    ...
    if M.isEpub(file) then
        local Epub = require("qui_metadata.qm_epub")
...
function M.canRestore(file)
    ...
    local available, err = require("qui_metadata.qm_epub").canRestore(file)
    local cache_ok, cache_err = refresh_hash_sidecar(file)
