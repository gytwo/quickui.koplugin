...
local XML = require("qui_metadata.qm_slaxdom")
...
local BACKUP_SUFFIX = ".quickui-metadata.bak"
local TRANSACTION_VERSION = 1
local SPACE_MARGIN = 1024 * 1024
local SIDECAR_NAME = "/custom_metadata.lua"
local SIDECAR_NEW_SUFFIX = ".qui-metadata.new"
local SIDECAR_OLD_SUFFIX = ".qui-metadata.old"
...
local function paths(path)
    ...
    return {
        ...
        stage = path .. ".quickui-metadata.tmp",
        restore_stage = path .. ".quickui-metadata.restore",
        backup_new = backup .. ".new",
        backup_old = backup .. ".old",
        companion_new = backup .. ".json.new",
        companion_old = backup .. ".json.old",
        marker = path .. ".quickui-metadata.txn",
        marker_new = path .. ".quickui-metadata.txn.new",
        marker_old = path .. ".quickui-metadata.txn.old",
        live_old = path .. ".quickui-metadata.live-old",
        scratch = path .. ".quickui-metadata.entry",
    }
end
...
local function unique_series_id(metadata)
    ...
    local base = "quickui-metadata-series"
...
local function scan_archive(path, capture, verify_all, scratch_path)
    ...
    if verify_all then
        scratch_path = scratch_path or path .. ".quickui-metadata.entry"
        local removed
